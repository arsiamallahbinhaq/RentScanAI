      ...
      return (
        <div className="max-w-7xl mx-auto px-6 pt-16">
          <section className="mb-20">
            {/* Top Control Bar with standard, humble, literal labels */}
            <div className="flex flex-col sm:flex-row justify-between items-center bg-slate-900 border border-slate-800 px-8 py-5 rounded-[2rem] gap-4 mb-8 shadow-2xl animate-fade-in">
              <button 
                onClick={() => { 
                  setResult(null); 
                  setError(null);
                ...
                disabled={loading}
                className="flex items-center gap-2 text-emerald-400 hover:text-emerald-300 disabled:text-slate-600 font-bold uppercase tracking-widest text-xs transition-all active:scale-95 cursor-pointer disabled:cursor-not-allowed"
              >
                <ArrowRight className="w-4 h-4 rotate-180" />
                Kembali ke Pencarian & List
              </button>
              <div className="flex items-center gap-2">
                <span className={`w-2 h-2 rounded-full ${loading ? 'bg-amber-400 animate-pulse' : 'bg-emerald-500'}`}></span>
                <span className="text-slate-400 font-mono text-[10px] uppercase font-black tracking-widest">
                  {loading ? "Sedang Menganalisis Iklan..." : "Hasil Investigasi Kelayakan"}
                </span>
              </div>
            </div>
            ...
            {loading ? (
              <motion.div 
                ...
                className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-12 text-center max-w-2xl mx-auto shadow-2xl relative overflow-hidden"
              >
                ...
                <h3 className="text-2xl font-black text-white mb-3 uppercase italic tracking-tight">Investigasi RentScan...</h3>
                <p className="text-slate-400 max-w-md mx-auto mb-8 font-medium">
                  Kami sedang membedah properti ini. Pengecekan mencakup kewajaran harga wilayah, korelasi teks deskripsi, kelengkapan data, dan indikasi manipulasi scam.
                </p>

                <div className="inline-flex items-center gap-3 bg-slate-950 px-6 py-3 rounded-full border border-slate-800">
                  <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />
                  <span className="text-xs font-mono font-bold uppercase tracking-wider text-emerald-400">{loadingStep}</span>
                </div>
                ...
                {error && (
                  <motion.div 
                    ...
                    className="mt-8 bg-rose-500/10 border border-rose-500/20 p-4 rounded-2xl flex items-center gap-3 text-left"
                  >
